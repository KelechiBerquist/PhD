...
int main ()
{
	...
	for (int chosen = 0;  chosen < MainVector.size();  chosen++)
	{
		...
		for (int forDirection = 0;  forDirection < Direction.size() ;  forDirection++)
		{
			...
			string  RunSimulationFolderName   =  RunPC + itos(chosen+1) + "/" + Direct + "/UnLoad/";
			CurrentSystem.readFolderName  = RunPC + itos(chosen+1) + "/" +  Direct + "/RetrievedCoords/";
			...
			WriteFileName2   =  CurrentSystem.writeFolderName + "runRelax_Chosen" + "_" +  Direct + itos(chosen+1);
			...
			for (int forRate = 0;  forRate < StrainRate.size();  forRate++)
			{
				...
				for (int timeDump = StrainRestart;  timeDump <= Strain; timeDump+=2)
				{
					...
					CurrentSystem.readFileNameEnd  = string(1,CurrentSystem.direction[0]) + "-Rate" + dtos(CurrentSystem.rate) + "-Spn" + itos(chosen+1) + "-S" + itos(CurStrain);
					CurrentSystem.fileNameEnd  = "UnLoad-Rate" + dtos(CurrentSystem.rate) + "-Spn" + itos(chosen+1) + "-S" + itos(CurStrain);
					
					cout<<string(1, CurrentSystem.direction[0])<<"   ";
					WriteFileName3 = CurrentSystem.readFolderName + "InConfig_" + CurrentSystem.readFileNameEnd +".dat";
					
					
					WriteFileName1   =  CurrentSystem.writeFolderName  + "Relax-Rate" + dtos(CurrentSystem.rate) + "_S" + itos(CurStrain) + "_" + CurrentSystem.direction[0] + ".pm";
